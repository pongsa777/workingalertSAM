...
if($userid == 0){
	...
	$data = array();
    $que = "SELECT * FROM `group` WHERE `group`.`group_name` like '%$searchmsg%' LIMIT 0 , 30;";
	$querygroup = $con->query($que);
	if($querygroup->num_rows > 0){
		while($groupdata = $querygroup->fetch_assoc()){
			array_push($data,array("groupid"=>$groupdata['group_id'],"groupname"=>$groupdata['group_name'],"lock"=>$groupdata['password']));
		}
