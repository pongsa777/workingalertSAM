...
if($userid != 0){
  if($groupid != "" || $groupid != 0 || $memberid != "" || $memberid != 0){
    ...
    if ($querycheckadmin->num_rows > 0){
      //เช็คว่า user เป็นสมาชิกในกลุ่มและเป็น admin อยู่
      $sqlcheckmember = "SELECT * FROM `has_user` WHERE `role_id` = '1' AND `user_id` = '$memberid' AND `has_user`.`group_id` = '$groupid'";
      $querycheckmember = $con->query($sqlcheckmember);
      if ($querycheckmember->num_rows > 0) {
        //เช็คว่าไม่ได้ remove ตัวเองออก
        if($userid != $memberid){
          ...
          if($con->query($sqlupdatestatus)){
            $response = array("status"=>"success","description"=>"remove admin success");
          }else {
            $response = array("status"=>"failed","description"=>"update database failed");
          }
        }else{
          $response = array("status"=>"failed","description"=>"You can not remove yourself");
        }
      }else {
        $response = array("status"=>"failed","description"=>"user is not in this group or user is not admin status");
      }
    }else {
