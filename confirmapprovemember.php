...
if($userid != 0){
  if($groupid != "" || $groupid != 0 || $memberid != "" || $memberid != 0){
    ...
    if ($querycheckadmin->num_rows > 0) {
      //เช็คว่า memberid เป็นสถานะรอ admin จริง
      $sqlcheckmember = "SELECT * FROM `has_user` WHERE `role_id` = '4' AND `user_id` = '$memberid' AND `has_user`.`group_id` = '$groupid'";
      ...
      if ($querycheckmember->num_rows > 0) {
        ...
        if($con->query($sqlupdatestatus)){
          $response = array("status"=>"success","description"=>"confirm by admin success");
        }else {
          $response = array("status"=>"failed","description"=>"update database failed");
        }
      }else {
        $response = array("status"=>"failed","description"=>"user status is not in pending admin confirm");
      }
    }else{
