  ...
  if($groupid != "" || $groupid != 0 || $memberid != "" || $memberid != 0){
    ...
    if ($querycheckadmin->num_rows > 0){
      ...
      if ($querycheckmember->num_rows > 0) {
        $sqlupdatestatus = "UPDATE  `workingalert`.`has_user` SET  `role_id` =  '3' WHERE  `has_user`.`user_id` ='$memberid' AND `has_user`.`group_id` = '$groupid'";
        if($con->query($sqlupdatestatus)){
          $response = array("status"=>"success","description"=>"block user success");
        }else {
          $response = array("status"=>"failed","description"=>"update database failed");
        }
      }else {
        $response = array("status"=>"failed","description"=>"user is not in this group or user is already block");
      }
    }else {
